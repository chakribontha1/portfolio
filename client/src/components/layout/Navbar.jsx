  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${
        scrolled ? 'glass-nav border-b border-ink/6' : ''
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12 h-[64px] flex items-center justify-between">
          ...
          {navLinks.map(l => (
            <a key={l.href} href={l.href} onClick={e => { e.preventDefault(); go(l.href) }}
              className={`relative px-4 py-2 font-body text-sm rounded-lg transition-all duration-200 ${
                active === l.href.slice(1) ? 'text-accent bg-accent/8' : 'text-ink-2 hover:text-ink hover:bg-ink/4'
              }`}>
              {l.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:block">
            <motion.a href="mailto:bonthachakri1@gmail.com"
              className="px-5 py-2 rounded-xl border border-accent/30 text-accent font-display font-semibold text-sm hover:bg-accent hover:text-accent-contrast transition-all duration-250"
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            className="md:hidden glass-nav border-b border-ink/6 px-6 py-4 flex flex-col gap-1">
            {navLinks.map((l, i) => (
              <motion.a key={l.href} href={l.href} onClick={e => { e.preventDefault(); go(l.href) }}
                initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                className="py-3 px-4 font-body text-ink-2 hover:text-accent hover:bg-ink/4 rounded-lg transition-all">
                {l.label}
              </motion.a>
            ))}
            <a href="mailto:bonthachakri1@gmail.com" className="mt-2 py-3 text-center bg-accent text-accent-contrast font-display font-bold text-sm rounded-xl">
              Hire Me
            </a>
          </motion.div>
        )}
