...
const socials = [
  ...
  { icon: Linkedin, label: 'LinkedIn', href: 'https://www.linkedin.com/in/bonthachakri/', val: 'linkedin.com/in/bonthachakri', color: '#38BDF8' },
  { icon: Mail,     label: 'Email',    href: 'mailto:bonthachakri1@gmail.com',            val: 'bonthachakri1@gmail.com',    color: 'var(--accent)' },
]
...
export default function Contact() {
  ...
  const field = `w-full bg-ink/4 border border-ink/8 rounded-xl px-4 py-3 font-body text-sm text-ink placeholder-ink-3
    focus:outline-none focus:border-accent/40 focus:bg-ink/6 transition-all duration-200 font-body`
    ...
    <section id="contact" ref={ref} className="py-28 relative">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <motion.div {...anim(1)} className="mb-16">
          <h2 className="font-display font-extrabold text-ink leading-none mb-4 text-balance" style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)', letterSpacing: '-0.025em' }}>
            Let's build something<br /><span className="text-accent">great together.</span>
          </h2>
          <p className="font-body text-ink-2 text-[15px] max-w-xl">
            Open to backend engineering roles, and interesting systems-design conversations.
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          ...
          <motion.div {...anim(2)} className="lg:col-span-3">
            <form onSubmit={onSubmit} className="glass space-y-4 p-8 rounded-3xl">
              ...
              <div>
                ...
                <textarea name="message" value={form.message} onChange={onChange} placeholder="Tell me about the role or project..." required rows={5} className={`${field} resize-none`} />
              </div>

              {status === 'success' && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  className="flex items-center gap-2.5 p-3.5 bg-green-500/10 border border-green-500/25 rounded-xl">
              ...
              {status === 'error' && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  className="flex items-center gap-2.5 p-3.5 bg-red-500/10 border border-red-500/25 rounded-xl">
              ...
              <motion.button type="submit" disabled={status === 'loading'}
                className="w-full py-3.5 bg-accent text-accent-contrast font-display font-bold text-sm rounded-xl flex items-center justify-center gap-2 hover:bg-accent-2 transition-all duration-200 disabled:opacity-50"
                style={{ boxShadow: '0 8px 24px var(--accent-soft)' }}
                whileHover={{ scale: 1.01, translateY: -1 }} whileTap={{ scale: 0.99 }}>
                {status === 'loading' ? (
                  <><div className="w-4 h-4 border-2 border-accent-contrast/30 border-t-accent-contrast rounded-full animate-spin" />Sending...</>
                ) : (
          ...
          <motion.div {...anim(3)} className="lg:col-span-2 flex flex-col gap-4">
            ...
            {socials.map(({ icon: Icon, label, href, val, color }, i) => (
              <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer"
                ...
                className="group glass glass-interactive flex items-center gap-4 p-5 rounded-2xl">
            ...
            {/* Available badge */}
            <div className="glass p-4 rounded-2xl flex items-center gap-3" style={{ borderColor: 'rgba(110,231,183,0.25)' }}>
              ...
              <div>
                ...
                <div className="font-mono text-[11px] text-ink-3 mt-0.5">Full-time · Hyderabad or Remote</div>
              </div>
            </div>
          </motion.div>
        </div>
