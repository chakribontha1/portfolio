    ...
    <section id="experience" ref={ref} className="py-28 relative">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 relative">
        <motion.div {...anim(0)} className="flex items-center gap-3 mb-12">
          <span className="font-mono text-[11px] text-accent uppercase tracking-[0.18em]">03 — Experience</span>
        ...
        <div className="max-w-4xl space-y-6">
          {experiences.map((e, idx) => (
            <motion.div key={e._id || e.company} {...anim(2 + idx)}
              ...
              {/* Tech */}
              <div className="flex flex-wrap gap-1.5 pt-5 border-t border-ink/6">
                {e.tech?.map(t => (
                  <span key={t} className="px-2.5 py-1 rounded-lg font-mono text-[10px] text-ink-3 bg-ink/4 border border-ink/7 hover:text-accent hover:border-accent/25 transition-all cursor-default">
                    {t}
                  </span>
                ))}
              </div>
